...
function ShpRecord(reader) {
  ...
  switch(this.shapeType) {
    ...
    case ShpType.SHAPE_POLYGONZ:
      this.shape = new ShpPolygonZ(reader, this.contentLengthBytes);
      break;
    case ShpType.SHAPE_POLYLINEZ:
      this.shape = new ShpPolylineZ(reader, this.contentLengthBytes);
      break;
...
function ShpPointZ(reader, size) {
  ...
  this.readZFromPolygonZOrPolylineZ=function(reader)
  {
...
function ShpPolygonZ(reader, size) {
  // for want of a super()
  ShpPolylineZ.apply(this, [reader, size]);
  this.type = ShpType.SHAPE_POLYGONZ;
}
function ShpPolylineZ(reader, size) {
  this.type = ShpType.SHAPE_POLYLINEZ;
  this.rings = [];
  if (reader) {
    if (reader.getBytesRemaining() < size){
      throw(new ShpError("Not a Polygon record (too small)"));
    ...
    var points = [];
    while(pc--) {
      points.push(new ShpPointZ(reader,16));
    }
    reader.skip(16);
    point_index=0;
    while(point_index<pcz)
    {
     points[point_index].readZFromPolygonZOrPolylineZ(reader);
     point_index++;
    }
    // convert points, and ringOffsets arrays to an array of rings:
    var removed = 0;
